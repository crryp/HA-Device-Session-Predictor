...
3. **At session end**, the duration, energy use, and curve are saved into a
   small history (default: last 6 curves, last 20 durations/energy values).
...
5. **Duplicate detection**: a session that closely resembles an
   ...
   limited 6 curve slots from filling up with 6 copies of the same
   ...
   duration would lead you to expect).
...
- **255-character limit.** The curve storage uses `input_text` helpers (HA
  ...
  sessions you can keep. The defaults (6 fine + 9 coarse, last 6 sessions)
  fit comfortably, but if you extend this, keep an eye on the limit.
...
- Fallback grace period (5 minutes before switching to the second-best
  match)
- How many sessions/curves are kept (default 20 and 6 respectively)
